...
function Layout({ children }) {
  ...
  // Load products list state from localStorage, default to true
  const [showProductsList, setShowProductsList] = useState(() => {
    const saved = localStorage.getItem('showProductsList')
  ...
  useEffect(() => {
    ...
    else if (path === '/orders') setActiveMenu('Orders')
    else if (path === '/products') setActiveMenu('Products')
    else if (path === '/returns') setActiveMenu('Returns')
    else if (path === '/customers') setActiveMenu('Customers')
    else if (path === '/analytics') setActiveMenu('Analytics')
  ...
  // Ensure products list is visible on mount
  useEffect(() => {
    const saved = localStorage.getItem('showProductsList')
    if (saved === null) {
      setShowProductsList(true)
      localStorage.setItem('showProductsList', JSON.stringify(true))
    }
  }, [])
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response && response.success && response.data && Array.isArray(response.data)) {
        const productsList = response.data.slice(0, 10) // Show only first 10 products in sidebar
        console.log('Setting products:', productsList.length) // Debug log
      ...
      console.error('Error fetching products for sidebar:', err)
      setProducts([])
    } finally {
    ...
    <div className="layout-container">
      ...
      <div className={`dashboard-sidebar ${isSidebarOpen ? 'open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          ...
          {/* Products List Section - Always Visible */}
          <div className="products-sidebar-section">
            <div 
              className="products-toggle"
              onClick={() => {
                const newState = !showProductsList
                setShowProductsList(newState)
                localStorage.setItem('showProductsList', JSON.stringify(newState))
              }}
            >
              <svg className="nav-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M9 18L15 12L9 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ transform: showProductsList ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}/>
              </svg>
              <span>Products List</span>
            </div>
            <div className="products-list-container" style={{ display: showProductsList ? 'block' : 'none' }}>
              {productsLoading ? (
                ...
                <div className="products-list">
                  {products.map((product) => (
                    <div key={product.id || product._id || Math.random()} className="product-sidebar-item">
                      {product.images && product.images.length > 0 ? (
                        <img 
                          src={product.images[0].src} 
                          alt={product.title}
                          className="product-sidebar-image"
                        />
                      ) : (
                        <div className="product-sidebar-placeholder">
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M20 7H4C2.89543 7 2 7.89543 2 9V19C2 20.1046 2.89543 21 4 21H20C21.1046 21 22 20.1046 22 19V9C22 7.89543 21.1046 7 20 7Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                          </svg>
                        </div>
                      )}
                      <span className="product-sidebar-title" title={product.title}>
                        {product.title && product.title.length > 25 ? product.title.substring(0, 25) + '...' : (product.title || 'Untitled Product')}
                      </span>
                    </div>
                  ))}
                  {products.length >= 10 && (
                    <div 
                      className="product-sidebar-item view-all"
          ...
          <div 
            ...
            onClick={() => handleMenuClick('Returns', '/returns')}
          >
            <svg className="nav-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
