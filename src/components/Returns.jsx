...
function Returns() {
  ...
  const [activeMenu, setActiveMenu] = useState('Returns')
  ...
  return (
    <div className="returns-container">
      ...
      {/* Left Sidebar */}
      <div className={`returns-sidebar ${isSidebarOpen ? 'open' : ''}`}>
                  ...
                  {filteredReturns.length > 0 ? (
                    filteredReturns.map((returnItem, index) => (
                      <tr key={index}>
                        ...
                        <td data-label="Customer">{returnItem.customer?.name || returnItem.customer}</td>
                        <td data-label="Product">{returnItem.product?.name || returnItem.product}</td>
                        <td data-label="Reason">
                          <span className="reason-tag" style={{ color: getReasonColor(returnItem.reason) }}>
                            {returnItem.reason}
                          </span>
                        </td>
                        <td data-label="Date">{returnItem.date || new Date(returnItem.createdAt).toISOString().split('T')[0]}</td>
                        <td data-label="Amount">${returnItem.amount?.toFixed(2)}</td>
                    ...
                    <tr>
                      <td colSpan="7" className="no-returns">No returns found</td>
                    </tr>
                  )}
                </tbody>
              </table>
