...
function Settings() {
  ...
  const [wooCommerceWebhookUrl, setWooCommerceWebhookUrl] = useState('')
  ...
  const fetchWooCommerceStatus = async () => {
    try {
      ...
      if (response.data) {
        ...
        // Set webhook URL if available, or generate it from secret key
        if (response.data.webhookUrl) {
          setWooCommerceWebhookUrl(response.data.webhookUrl)
        } else if (response.data.secretKey) {
          // Generate webhook URL from secret key
          ...
          if (window.location.origin.includes('localhost:5173') || window.location.origin.includes('localhost:3000')) {
            ...
            backendUrl = 'https://backo-server.vercel.app'
          }
          const webhookUrl = `${backendUrl}/api/webhook/woocommerce/${response.data.secretKey}`
          setWooCommerceWebhookUrl(webhookUrl)
        }
      }
    } catch (err) {
  ...
  const handleWooCommercePortalConnect = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Generate webhook URL if secret key is available
        if (secretKey) {
          ...
          const webhookUrl = `${backendUrl}/api/webhook/woocommerce/${secretKey}`
          setWooCommerceWebhookUrl(webhookUrl)
        }
        // Also check if webhook URL is in response
        if (response.data.webhookUrl) {
          setWooCommerceWebhookUrl(response.data.webhookUrl)
        }
                  ...
                  {wooCommerceWebhookUrl && (
                    <div className="form-group" style={{ 
                      ...
                      <label className="form-label" style={{ fontWeight: 600, color: '#2C2C2C', marginBottom: '0.5rem' }}>
                        🔗 Webhook URL (Use this in WordPress Plugin)
                      </label>
                      <p style={{ fontSize: '0.875rem', color: '#666', marginBottom: '0.5rem' }}>
                        WordPress plugin se orders, products, aur customers ka data is URL par automatically send hoga
                      </p>
                      <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            navigator.clipboard.writeText(wooCommerceWebhookUrl)
                            alert('Webhook URL copied to clipboard!')
                          }}
                          style={{ whiteSpace: 'nowrap', padding: '0.5rem 1rem', background: '#4CAF50', color: '#FFFFFF', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '0.875rem' }}
                        >
                      ...
                      <p style={{ fontSize: '0.75rem', color: '#666', marginTop: '0.5rem', fontStyle: 'italic' }}>
                        WordPress plugin mein ye webhook URL configure karo - automatically orders, products, aur customers sync honge
                      </p>
                    </div>
                  )}
